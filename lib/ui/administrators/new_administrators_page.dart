...
class NewAdministratorsPage extends StatefulWidget {
  static const route = "/new_users_page";

  final roles role;

  const NewAdministratorsPage({required this.role});

  @override
  State<NewAdministratorsPage> createState() => _NewAdministratorsPageState();
}
...
class _NewAdministratorsPageState extends State<NewAdministratorsPage> {
              ...
              setState(() {
                loadMore = false;
                  if (state.administratorsWithPagination.paginationInfo != null)
                  {
                    ...
                    if (state.administratorsWithPagination.paginationInfo!.pageNo == 1)
                      users = [];
...
class _recordDialogState extends State<_recordDialog> {
  ...
  Future<Company> pickCompany(Company selectedCompany) async {
        ...
        builder: (BuildContext context) {
          return BlocProvider<UserBloc>.value(
              ...
              AlertDialog(
                ...
                content: setupCompaniesPickerDialog(selectedCompany),
                actions: [
                  Padding(
                    child: CustomButton(
...
class CompaniesWidget extends StatefulWidget {
  ...
  const CompaniesWidget(
      {Key? key, required this.selectedCompany, required this.state, required this.bloc})
      : super(key: key);
